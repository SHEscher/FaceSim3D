...
namespace UXF
{
    ...
    public class WebAWSDynamoDB : DataHandler
    {
        ...
        [DllImport("__Internal")]
        // private static extern void DDB_BatchGetItem(string tableName, string projectionExpression, string status, string callbackGameObjectName, string guid);
        private static extern void DDB_BatchGetItem(string tableName, string jsonRequests, string callbackGameObjectName, string guid, string projectionExpression);

        [DllImport("__Internal")]
        private static extern void DDB_Query(string tableName, string callbackGameObjectName, string guid, string keyConditionExpression, string expressionAttributeNames=null,
                        string expressionAttributeValues=null, string projectionExpression=null, string exclusiveStartKey=null, string filterExpression=null);
        ...
        private static void DDB_BatchGetItem(string tableName, string jsonRequests, string callbackGameObjectName, string guid, string projectionExpression) => Error();
        private static void DDB_Query(string tableName, string callbackGameObjectName, string guid, string keyConditionExpression, string expressionAttributeNames = null,
                      string expressionAttributeValues=null, string projectionExpression=null, string exclusiveStartKey=null, string filterExpression = null) => Error();
        ...
        private const string sortKey = "SystemDateTime_BeginTrial"; //"trial_num";
        ...
        private Dictionary<string, Action<Dictionary<string, object>>> queryCallbackMap = new Dictionary<string, Action<Dictionary<string, object>>>();
        ...
        public override string HandleJSONSerializableObject(Dictionary<string, object> serializableObject, string experiment, string ppid, int sessionNum, string dataName, UXFDataType dataType, int optionalTrialNum)
        {
            ...
            {
                if (!serializableObject.ContainsKey(sortKey))
        ...
        public void GetCustomBatchDataFromDB(string tableName, string primaryKeyName, object primaryKeyValue, Action<Dictionary<string, object>> callback,
            string keyName, object keyValue, string projectionExpression,
            string optionalSortKeyName = "", object optionalSortKeyValue = null)
        {
            bool ok = CheckCurrentTargetOK();
            if (!ok) return;
            ...
            request.Add(keyName, keyValue);
            string guid = Guid.NewGuid().ToString();
            string jsonRequests = MiniJSON.Json.Serialize(request);
            DDB_BatchGetItem(tableName, jsonRequests, gameObject.name, guid, projectionExpression);
            requestCallbackMap.Add(guid, callback);
        }


        public void QueryCustomBatchDataFromDB(string tableName, string primaryKeyName, object primaryKeyValue, Action<Dictionary<string, object>> callback,
            string keyName, object keyValue, string projectionExpression, string exclusiveStartKey = "",
            string optionalSortKeyName = "", object optionalSortKeyValue = null)
        {
            bool ok = CheckCurrentTargetOK();
            if (!ok) return;
            ...
            DDB_Query(tableName, gameObject.name, guid,
                      "#pkn = :pkv",
                      "{ \"#pkn\": \"" + primaryKeyName + "\", \"#key\": \"" + keyName + "\"}",
                      "{ \":pkv\": \"" + primaryKeyValue + "\", \":val\": \"" + keyValue + "\"}",
                      projectionExpression, exclusiveStartKey,
                      "#key = :val");
            queryCallbackMap.Add(guid, callback);
        }
        ...
        private void HandleSuccessfulDBQuery(string jsonResult)
        {
            //Utilities.UXFDebugLog(jsonResult);
            Dictionary<string, object> jsonRequest = (Dictionary<string, object>)MiniJSON.Json.Deserialize(jsonResult);
            string guid = (string)jsonRequest["guid"];
            //string lastEvaluatedKey = MiniJSON.Json.Serialize((Dictionary<string, object>)jsonRequest["lastEvaluatedKey"]);
            object lastKeyValue; string lastEvaluatedKey;
            if (jsonRequest.TryGetValue("lastEvaluatedKey", out lastKeyValue))
            {
                lastEvaluatedKey = MiniJSON.Json.Serialize((Dictionary<string, object>)lastKeyValue);
            }
            else
            {
                lastEvaluatedKey = "none";
            }
            //Debug.Log($"HandleSuccessfulDBQuery lastEvaluatedKey: {lastEvaluatedKey}");

            List<object> jsonResults = ((IEnumerable)jsonRequest["result"]).Cast<object>().ToList();
            Action<Dictionary<string, object>> callback = queryCallbackMap[guid];
        ...
        private void HandleUnsuccessfulDBQuery(string jsonResult)
        {
            Dictionary<string, object> jsonRequest = (Dictionary<string, object>)MiniJSON.Json.Deserialize(jsonResult);
            string guid = (string)jsonRequest["guid"];
            Action<Dictionary<string, object>> callback = queryCallbackMap[guid];
        ...
        private void HandleBeforeUnloadEvent()
        {
            if (endSessionWhenTryCloseTab)
            {
                ...
                // Consider the experiment cancelled
                GameObject oddOneOutTask = GameObject.Find("OddOneOutTask");
                OddOneSelector redirectScript = oddOneOutTask.GetComponent<OddOneSelector>();
                redirectScript.exit_exp = true;

                // Reset unused triplets
                foreach (string triplet in GenerateExperiment.currentBlockDB.Keys)
                {
                    if ((string)((Dictionary<string, object>)GenerateExperiment.currentBlockDB[triplet])["status"] == "L")
                    {
                        Debug.Log($"Releasing {triplet} before quit");
