...
def parseargs():
    ...
    aa("--epochs", metavar="T", type=int, default=2000, help="maximum number of epochs to run VICE optimization")
    aa("--burnin", type=int, default=500, help="minimum number of epochs to run VICE optimization")
    aa("--eta", type=float, default=0.001, help="learning rate to be used in optimizer")
    aa("--init_dim", metavar="D", type=int, default=100, help="initial dimensionality of the latent space")
    aa(
        "--batch_size",
        metavar="B",
        type=int,
    ...
    aa("--optim", type=str, default="adam", choices=["adam", "adamw", "sgd"], help="optimizer to train VICE")
    aa(
        "--prior",
        type=str,
        metavar="p",
    ...
    aa("--mc_samples", type=int, default=10, help="number of weight samples to use for MC sampling")
    aa("--spike", type=float, default=0.25, help="sigma for spike distribution")
    aa("--slab", type=float, default=1.0, help="sigma for slab distribution (should be smaller than spike)")
    aa(
        "--pi",
        type=float,
        default=0.5,
    ...
    aa("--steps", type=int, default=50, help="perform validation and save model parameters every <steps> epochs")
    aa("--device", type=str, default="cpu", help="whether training should be performed on CPU or GPU (i.e., CUDA).")
    aa(
        "--num_threads",
        type=int,
        default=4,
