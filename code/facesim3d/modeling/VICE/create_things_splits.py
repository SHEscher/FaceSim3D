...
def parseargs():
    ...
    aa("--triplets_dir", type=str, help="triplets directory (parent directory for full triplet dataset)")
    aa("--n_folds", type=int, default=10, choices=[40, 20, 10, 5], help="split data into k number of folds")
    aa(
        "--additional_fractions",
        type=int,
        nargs="+",
        default=None,
...
def get_splits(main_path: str, p: int = 10) -> list[str]:
    ...
    return sorted(
        [os.path.join(split_path, d.name) for d in os.scandir(split_path) if d.is_dir() and d.name[-2:].isdigit()]
    )
