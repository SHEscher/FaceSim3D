...
def parseargs():
    ...
    aa("--n_objects", type=int, default=1854, help="number of unique items/objects in dataset")
    aa("--init_dim", type=int, default=100, help="initial dimensionality of VICE latent space")
    ...
    aa("--triplets_dir", type=str, help="directory from where to load validation and held-out test set")
...
def inference(
    ...
    for seed, vice, vice_path in zip(seeds, vice_models, vice_paths, strict=False):
        pruned_vice = pruning(vice)
        val_loss, _ = pruned_vice.evaluate(val_batches)
        test_acc, test_loss, probas, vice_pmfs, triplet_choices = pruned_vice.inference(test_batches)
    ...
    seeds, _ = zip(*sorted(val_losses.items(), key=lambda kv: kv[1], reverse=False), strict=False)
    ...
    for seed, vice_pmfs in vice_pmfs_all.items():
        ...
        cross_entropies[seed] = np.mean(compute_divergences(human_pmfs, vice_pmfs, metric="cross-entropy"))
        l1_distances[seed] = np.mean(compute_divergences(human_pmfs, vice_pmfs, metric="l1-distance"))
