...
import pandas as pd
import torch
from facesim3d.modeling.VICE.data_loader import DataLoader
...
def load_data(
    ...
    if inference:
        # val_set replaced "test_triplets"
        with open(pjoin(triplets_dir, f"{val_set}.npy"), "rb") as test_triplets:
    ...
    try:
        ...
        test_triplets = (
            torch.from_numpy(np.loadtxt(pjoin(triplets_dir, f"{val_set}.txt"))).to(device).type(torch.LongTensor)
        )
...
def load_batches(
    ...
    if inference:
        ...
        test_batches = DataLoader(dataset=test_triplets, n_objects=n_objects, batch_size=batch_size, train=False)
        return test_batches
    # create two iterators of train and validation mini-batches respectively
    train_batches = DataLoader(dataset=train_triplets, n_objects=n_objects, batch_size=batch_size, train=True)
    val_batches = DataLoader(dataset=test_triplets, n_objects=n_objects, batch_size=batch_size, train=False)
