...
import numpy as np
import torch
from facesim3d.modeling.VICE import optimization, utils
...
def parseargs():
    ...
    aa("--n_objects", type=int, help="number of unique objects/items/stimuli in dataset")
    aa("--modality", type=str, metavar="o", help="Modality e.g., behavioral, text, visual, fMRI")
    aa("--init_dim", type=int, default=100, help="initial latent dimensionality of VICE embedding(s)")
    aa(
        "--thresh",
        type=float,
        default=0.8,
    ...
    aa("--batch_size", metavar="B", type=int, default=128, help="number of triplets in each mini-batch")
    aa(
        "--optim",
        type=str,
        metavar="o",
    ...
    aa("--rnd_seed", type=int, default=42, help="random seed for reproducibility of results")
        ...
        for j, W_mu_j in enumerate(Ws_mu):
            if i != j:
                ...
                if Ws_sigma:
                    ...
                    scale_robustness_scores.append(uncertainty_estimates(W_sigma_i_test, sorted_dims, rel_freq))
...
def compute_robustness(Ws_mu: list, Ws_sigma: list | None = None, thresh: float = 0.9):
    avg_loc_robustness, std_loc_robustness, avg_scale_robustness = compare_dimensions(Ws_mu, thresh, Ws_sigma)
    scores = {}
...
def evaluate_models(
    ...
    in_path = os.path.join(results_dir, modality, f"{init_dim}d", optim, prior, str(spike), str(slab), str(pi))

    print(f"{triplets_dir = }")  # TODO: remove after testing
    print(f"{in_path = }")  # TODO: remove after testing
    ...
    print(f"{model_paths = }")  # TODO: remove after testing
    ...
    model_robustness_best_subset = compute_robustness(Ws_mu=pruned_locs, Ws_sigma=pruned_scales, thresh=thresh)
    ...
    final_embedding_pruned[:, np.argsort(-np.linalg.norm(final_embedding_pruned, axis=0, ord=1))]
