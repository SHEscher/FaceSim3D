...
class SpikeandSlab(nn.Module):
    ...
    def norm_pdf(X: Tensor, loc: Tensor, scale: Tensor) -> Tensor:
        ...
        return torch.exp(-((X - loc) ** 2) / (2 * scale.pow(2))) / scale * math.sqrt(2 * math.pi)
    ...
    def initialize_priors_(self) -> None:
        ...
        self.scale_spike = torch.ones(self.n_objects, self.init_dim).mul(self.spike).to(self.device)
        self.scale_slab = torch.ones(self.n_objects, self.init_dim).mul(self.slab).to(self.device)
