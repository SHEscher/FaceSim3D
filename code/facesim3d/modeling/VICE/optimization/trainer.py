...
from facesim3d.modeling.VICE import utils
...
class Trainer(nn.Module):
    ...
    def load_checkpoint_(self) -> None:
        ...
        if os.path.exists(self.model_dir):
            models = sorted([m.name for m in os.scandir(self.model_dir) if m.name.endswith("tar")])
            if len(models) > 0:
                try:
    ...
    def initialize_optim_(self) -> None:
        if self.optim == "adam":
            self.optim = torch.optim.Adam(self.parameters(), eps=1e-08, lr=self.eta)
        elif self.optim == "adamw":
            self.optim = torch.optim.AdamW(self.parameters(), eps=1e-08, lr=self.eta)
        elif self.optim == "sgd":
            self.optim = torch.optim.SGD(self.parameters(), lr=self.eta, momentum=0.9)
        else:
    ...
    def break_ties(probas: Tensor) -> Tensor:
        return torch.tensor(
            [-1 if torch.unique(pmf).shape[0] != pmf.shape[0] else torch.argmax(pmf) for pmf in probas]
        )
    ...
    def inference(
        ...
        for j, batch in enumerate(test_batches):
            ...
            if batch_probas.shape[0] < self.batch_size:
                ...
                human_choices = batch.nonzero(as_tuple=True)[-1].view(B, -1).cpu().numpy()
            else:
                probas[j * self.batch_size : (j + 1) * self.batch_size] += batch_probas
                human_choices = batch.nonzero(as_tuple=True)[-1].view(self.batch_size, -1).cpu().numpy()
            model_choices = utils.collect_choices(batch_probas, human_choices, model_choices)
    ...
    def fit(self, train_batches: Iterator, val_batches: Iterator) -> None:
        ...
        for epoch in range(self.start, self.epochs):
            ...
            batch_llikelihoods, batch_closses, batch_losses, batch_accs = self.stepping(train_batches)
    ...
    def save_final_latents(self) -> None:
        ...
        self.pruned_loc = pruned_loc[:, np.argsort(-np.linalg.norm(pruned_loc, axis=0, ord=1))]
        self.pruned_scale = pruned_scale[:, np.argsort(-np.linalg.norm(pruned_loc, axis=0, ord=1))]
