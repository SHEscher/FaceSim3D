...
def load_inds_and_item_names(folder: str = "./data") -> tuple[np.ndarray, np.ndarray]:
...
def load_ref_images(img_folder: str, item_names: np.ndarray) -> np.ndarray:
    ref_images = np.array(
        [
            resize(io.imread(pjoin("./reference_images", name + ".jpg")), (400, 400), anti_aliasing=True)
...
def load_data(device: torch.device, triplets_dir: str, inference: bool = False) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    try:
        ...
        train_triplets = (
            torch.from_numpy(np.loadtxt(pjoin(triplets_dir, "train_90.txt"))).to(device).type(torch.LongTensor)
        )
        test_triplets = (
            torch.from_numpy(np.loadtxt(pjoin(triplets_dir, "test_10.txt"))).to(device).type(torch.LongTensor)
        )
...
def load_batches(
    ...
    if multi_proc and n_gpus > 1:
        if sampling_method == "soft":
            warnings.warn(
                "...Soft sampling cannot be used in a multi-process distributed training " "setting.", RuntimeWarning
            )
            warnings.warn(
                "...Processes will equally distribute the entire training dataset amongst " "each other.",
                RuntimeWarning,
            )
            warnings.warn(
                "...If you want to use soft sampling, you must switch to single GPU or CPU " "training.", UserWarning
            )
...
def l2_reg_(model, weight_decay: float = 1e-5) -> torch.Tensor:
    loc_norms_squared = 0.5 * (model.encoder_mu[0].weight.pow(2).sum() + model.encoder_mu[0].bias.pow(2).sum())
...
def compute_similarities(
    anchor: torch.Tensor, positive: torch.Tensor, negative: torch.Tensor, method: str, distance_metric: str = "dot"
) -> tuple:
...
def collect_choices(probas: np.ndarray, human_choices: np.ndarray, model_choices: dict) -> dict:
    ...
    for pmf, choices in zip(probas, human_choices, strict=False):
        ...
        for j, batch in enumerate(val_batches):
            ...
            if sampling:
                ...
                model_choices = np.array(
                    [
                        np.random.choice(h_choice, size=len(p), replace=False, p=p)[::-1]
                        for h_choice, p in zip(human_choices, probas, strict=False)
                    ]
                )
...
def get_results_files(
    ...
    files = [
        pjoin(PATH, seed, f) for seed in os.listdir(PATH) for f in os.listdir(pjoin(PATH, seed)) if f.endswith(".json")
    ]
    return files
...
def pearsonr(u: np.ndarray, v: np.ndarray, a_min: float = -1.0, a_max: float = 1.0) -> np.ndarray:
    u_c = u - np.mean(u)
    v_c = v - np.mean(v)
    num = u_c @ v_c
...
def cross_correlate_latent_dims(X, thresh: float | None = None) -> float:
