# # Training script for SPoSE in main experiment within gender triplets

# Script must be run from project root "FaceSim3D":
# bash ./code/facesim3d/modeling/SPoSE/bash_for_training/training_spose_main_gender.sh
...
for gender in "female" "male"; do
  for cond in "2D" "3D"; do
    for lmbda in 0.005 0.01 0.05 0.1 0.5 1; do
      ...
      --results_dir ${p2results}/${cond}/${gender}/${modality}/${dims}d/${lmbda}/seed${seed} \
      --plots_dir ${p2results}/${cond}/${gender}/${modality}/${dims}d/${lmbda}/seed${seed}/Plots
